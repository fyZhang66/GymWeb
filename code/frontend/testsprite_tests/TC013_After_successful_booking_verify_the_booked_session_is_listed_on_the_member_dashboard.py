...
async def run_test():
    ...
    try:
        ...
        # -> Type 'member@gym.com' into the email field (input index 8).
        ...
        # -> Click on 'Book Sessions' in the main navigation to open the booking page.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/div/div/div/ul/div[2]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the first available time slot (the 09:00 button) to open the booking confirmation.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/main/div/div/div[2]/div/div/div[2]/div/div/div/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the 'Confirm Booking' button in the modal to complete the booking (then verify dashboard shows the new booked session).
        frame = context.pages[-1]
        # Click element
