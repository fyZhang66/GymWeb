...
async def run_test():
    ...
    try:
        ...
        # -> Fill the email field with the member account (member@gym.com), fill the password with 123456789, then click the Sign In button (will trigger login and navigation). ASSERTION: Using the test member credentials displayed on the page.
        ...
        # -> Click on 'Booking' in the main navigation to navigate to the booking page (use element index 171).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/div/div/div/ul/div[2]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the available 10:00 slot (Friday) to open the booking modal so notes can be entered and the session booked.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/main/div/div/div[2]/div/div/div[2]/div/div/div[2]/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Enter booking notes into the textarea and click 'Confirm Booking' to submit the booking.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div[3]/div/div/div/div[3]/div/textarea').nth(0)
        await asyncio.sleep(3); await elem.fill('Focus on upper body strength and improve form on bench press.')
