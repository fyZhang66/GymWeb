...
async def run_test():
    ...
    try:
        ...
        # -> Fill the email field with member@gym.com, fill the password field with 123456789, then click the 'Sign In' button.
        ...
        # -> Click the 'Weight Tracking' link in the main navigation to open the weight tracking page (use interactive element index 177).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/div/div/div/ul/div[4]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the 'Add weight entry' floating action button to open the add-entry form (index 446).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/main/div/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Fill the Weight (kg) field with a valid value and click 'Add Entry' to submit the weight entry.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div[3]/div/div/div/div/div[2]/div/div/input').nth(0)
        await asyncio.sleep(3); await elem.fill('74.6')
        ...
        current_url = await frame.evaluate("() => window.location.href")
        assert '/weight-tracking' in current_url
        assert await frame.locator("xpath=//*[contains(., 'Weight Tracking')]").nth(0).is_visible(), "Expected 'Weight Tracking' to be visible"
