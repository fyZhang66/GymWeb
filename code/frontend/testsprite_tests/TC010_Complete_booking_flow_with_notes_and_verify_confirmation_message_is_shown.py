...
async def run_test():
    ...
    try:
        ...
        # -> Fill the email and password fields with member account credentials and click the Sign In button.
        ...
        # -> Click on 'Book Sessions' (main navigation) to open the booking page and then verify available slots.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/div/div/div/ul/div[2]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the first available time slot (the 09:00 button for Friday, index 388) to open the booking dialog so notes can be entered and confirmation tested.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/main/div/div/div[2]/div/div/div[2]/div/div/div/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Input booking notes into the notes textarea (index 485) so the booking can be confirmed.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div[3]/div/div/div/div[3]/div/textarea').nth(0)
        await asyncio.sleep(3); await elem.fill('Focus on upper body strength and mobility. Please include progressive resistance and core stability work.')
