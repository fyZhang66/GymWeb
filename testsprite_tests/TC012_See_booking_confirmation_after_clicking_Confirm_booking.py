...
async def run_test():
    ...
    try:
        ...
        # -> Enter member credentials and submit the login form (fill Email and Password, then click Sign In).
        ...
        # -> Click on 'Book Sessions' in the main navigation to open the booking interface (element index 175).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/div/div/div/ul/div[2]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the first available time slot (the 09:00 button for Friday, March 20, index 392). After that, wait for the booking confirmation UI and then click the 'Confirm booking' button (if present).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/main/div/div/div[2]/div/div/div[2]/div/div/div/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the 'Confirm Booking' button (element index 502) to complete the booking and then verify the UI shows a visible booking confirmation message/state.
        frame = context.pages[-1]
        # Click element
